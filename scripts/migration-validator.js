...
const AustralianValidators = {
  ...
  validateGST: (totalAmount, gstAmount) => {
    ...
    return {
      ...
      error:
        difference > VALIDATION_CONFIG.tolerances.gstTolerance
  ...
  validateATOCategory: (category) => {
    ...
    return {
      ...
      error: validCategories.includes(category) ? null : `Invalid ATO category: ${category}`,
    };
  },
};
...
class MigrationValidator {
  ...
  async validateRecordCounts() {
    ...
    for (const [collection, tableName] of Object.entries(VALIDATION_CONFIG.collectionMapping)) {
      try {
        ...
        const hasDuplicates =
          duplicateResult.rows[0].total !== duplicateResult.rows[0].unique_count;
        ...
        if (hasDuplicates) {
          this.addIssue('DUPLICATE_RECORDS', `${tableName} contains duplicate records`, 'medium', {
            tableName,
          });
        }
  ...
  async validateDataIntegrity() {
    ...
    for (const collection of collections) {
      try {
        ...
        console.log(
          `   ${integrityIssues.length === 0 ? '✅' : '⚠️'} ${collection}: ${sampleSize} samples checked, ${integrityIssues.length} issues found`,
        );
      } catch (error) {
        console.error(`   ❌ Error validating ${collection}:`, error.message);
      }
  ...
  async validateAustralianCompliance() {
    ...
    for (const [check, config] of Object.entries(complianceChecks)) {
      try {
        ...
        console.log(
          `   ${validation.status} ${config.description}: ${invalidCount} invalid records`,
        );
      } catch (error) {
  ...
  async validateBankAccountsTable() {
    ...
    for (const check of checks) {
      try {
        ...
        console.log(
          `      ${invalidCount === 0 ? '✅' : '❌'} ${check.name}: ${invalidCount} invalid`,
        );
      } catch (error) {
        console.error(`      ❌ Error checking ${check.name}:`, error.message);
      }
  ...
  async validateReceiptsTable() {
    ...
    try {
      ...
      console.log(
        `      ${gstIssues === 0 ? '✅' : '❌'} GST calculations: ${gstIssues} issues in sample of ${result.rows.length}`,
      );
    ...
    try {
      ...
      console.log(
        `      ${invalidCount === 0 ? '✅' : '❌'} AI confidence scores: ${invalidCount} invalid`,
      );
    } catch (error) {
  ...
  async testConnectionPooling() {
    ...
    try {
      ...
      console.log(
        `      ✅ ${VALIDATION_CONFIG.performanceTests.concurrentConnections} concurrent connections: ${duration}ms`,
      );
  ...
  generateSummary() {
    ...
    results.summary.overallStatus =
      results.summary.criticalIssues === 0 && results.summary.totalIssues < 5 ? 'PASSED' : 'FAILED';

    // Success percentage
    results.summary.successRate =
      results.summary.totalCollections > 0
...
|--------------|------------------|--------|
${Object.entries(report.relationships)
  .map(([rel, val]) => `| ${rel} | ${val.orphanedRecords} | ${val.status} |`)
  .join('\n')}

## Data Integrity

${Object.entries(report.dataIntegrity)
  .map(
    ([collection, val]) => `
### ${collection}
...
${
  val.issues.length > 0
    ? `\nSample Issues:\n${val.issues
        .slice(0, 3)
        .map((i) => `- ${i.type}: ${i.message || JSON.stringify(i.issues)}`)
        .join('\n')}`
    : ''
}
`,
...
|-------|-----------------|--------|
${Object.entries(report.australianCompliance)
  .map(([check, val]) => `| ${val.check} | ${val.invalidRecords} | ${val.status} |`)
  .join('\n')}
...
${Object.entries(report.performance)
  .map(
    ([test, val]) => `| ${test} | ${val.duration || val.concurrentConnections}ms | ${val.status} |`,
  )
  .join('\n')}
...
${Object.keys(VALIDATION_CONFIG.collectionMapping)
  .reverse()
  .map((collection) => {
...
-- Verify rollback
${Object.values(VALIDATION_CONFIG.collectionMapping)
  .map((table) => `SELECT '${table}' as table_name, COUNT(*) as remaining_records FROM ${table};`)
  .join('\n')}
