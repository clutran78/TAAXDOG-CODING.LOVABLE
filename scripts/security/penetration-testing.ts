...
class PenetrationTestingService {
  ...
  private async testBruteForceProtection(): Promise<void> {
    ...
    this.addResult({
      ...
      details:
        blockedAt > 0 ? `Account locked after ${blockedAt} attempts` : 'No rate limiting detected',
  ...
  private async testSQLInjection(): Promise<void> {
    ...
    const endpoints = ['/api/users/profile', '/api/transactions/search', '/api/goals/filter'];
  ...
  private async testPrivilegeEscalation(): Promise<void> {
    ...
    for (const test of tests) {
      try {
        ...
        const isVulnerable =
          response.status === 200 &&
          (test.name === 'Role Manipulation' ? response.data?.role === 'ADMIN' : true);
  ...
  private async testCSRF(): Promise<void> {
    ...
    try {
      ...
      const response = await axios.post(
        `${this.baseUrl}/api/users/delete-account`,
        {},
        {
          headers: {
  ...
  private async testPasswordReset(): Promise<void> {
    ...
    const tests = [
      ...
      },
      {
        ...
        check: async () => {
          const validEmail = await axios.post(
          ...
          const invalidEmail = await axios.post(
            `${this.baseUrl}/api/auth/forgot-password`,
            {
          ...
          return {
            ...
            details: sameResponse
              ? 'Same response for valid/invalid emails'
              : 'Different responses reveal valid emails',
          };
        },
      },
    ];
  ...
  private async testPriceManipulation(): Promise<void> {
    ...
    try {
      ...
      const isVulnerable = response.status === 200 && response.data?.amount === 100; // $1 in cents
  ...
  private async testPathTraversal(): Promise<void> {
    ...
    for (const payload of payloads) {
      try {
        ...
        const isVulnerable =
          response.status === 200 &&
          (response.data?.includes('root:') || response.data?.includes('Administrator'));
  ...
  private async testAPIAccess(): Promise<void> {
    ...
    const publicEndpoints = ['/api/health', '/api/stripe/webhook', '/api/auth/providers'];

    const protectedEndpoints = ['/api/users/list', '/api/admin/stats', '/api/transactions/export'];
  ...
  private calculateSummary(): void {
    ...
    this.report.summary.vulnerable = this.report.results.filter(
  ...
  private displayResults(): void {
    ...
    for (const category of categories) {
      ...
      categoryResults.forEach((r) => {
        const icon = r.status === 'secure' ? '✅' : r.status === 'vulnerable' ? '❌' : '⚠️';
