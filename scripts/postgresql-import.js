...
class DatabaseImporter {
  ...
  buildInsertQuery(tableName, records, columns) {
    ...
    const query = `
      ...
      ${validColumns.filter(col => col !== 'id').map(col => `${col} = EXCLUDED.${col}`).join(', ')}
    `;
  ...
  async importCollection(collectionConfig, dataPath) {
    ...
    try {
      ...
      for (let i = 0; i < records.length; i += IMPORT_CONFIG.batchSize) {
        ...
        console.log(`  Progress: ${progress}% (${stats.imported} imported, ${stats.failed} failed)`);
      }
  ...
  async generateReport(outputDir) {
    ...
    const report = {
      ...
      successRate: (this.importStats.successfulImports / this.importStats.totalRecords * 100).toFixed(2) + '%'
    };
...
if (require.main === module) {
  ...
  const connectionString = process.env.DATABASE_URL || 'postgresql://genesis@localhost:5432/taaxdog_development';
