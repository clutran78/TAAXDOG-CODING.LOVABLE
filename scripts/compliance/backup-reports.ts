...
async function cleanupOldBackups(backupDir: string) {
  try {
    ...
    const archives = files.filter(f => f.startsWith('compliance-reports-') && f.endsWith('.tar.gz'));
    ...
    for (const archive of archives) {
      ...
      if (stats.mtime.getTime() < fourWeeksAgo) {
        ...
        const manifestName = archive.replace('.tar.gz', '.json').replace('compliance-reports-', 'manifest-');
        try {
