...
class OptimizedPostgreSQLImporter {
  ...
  calculateOptimalBatchSize(records) {
    ...
    const avgSize =
      sampleRecords.reduce((sum, record) => sum + JSON.stringify(record).length, 0) / sampleSize;
    ...
    return Math.min(Math.max(calculatedBatchSize, 1000), PERFORMANCE_CONFIG.maxBatchSize);
  }
  ...
  async processInParallel(
    records,
    processingFunction,
    workerCount = PERFORMANCE_CONFIG.parallelWorkers,
  ) {
  ...
  async importCollectionOptimized(config, dataPath) {
    ...
    try {
      ...
      if (copyResult !== null) {
        ...
        console.log(
          `   📈 Performance: ${Math.round(records.length / (duration / 1000))} records/second`,
        );
      ...
      console.log(
        `   📈 Performance: ${Math.round(results.successful / (duration / 1000))} records/second`,
      );
