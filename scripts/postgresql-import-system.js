...
class PostgreSQLImporter {
  ...
  validateRecord(record, config) {
    ...
    if (record.bank_account_id && !this.importedIds.get('bankAccounts')?.has(record.bank_account_id)) {
      errors.push({
  ...
  async importBatch(client, tableName, records, columns, config) {
    ...
    try {
      ...
      for (const record of records) {
        ...
        // Check for duplicates
        const isDuplicate = await this.checkDuplicates(client, tableName, record, config.uniqueFields);
        if (isDuplicate) {
          results.duplicates++;
          continue;
        }
    ...
    try {
      ...
      try {
        ...
        for (let i = 0; i < records.length; i += IMPORT_CONFIG.batchSize) {
          ...
          // Progress update
          if (i % (IMPORT_CONFIG.progressUpdateInterval * IMPORT_CONFIG.batchSize) === 0 || 
              i + IMPORT_CONFIG.batchSize >= records.length) {
            const progress = Math.round((i + batch.length) / records.length * 100);
            console.log(`   Progress: ${progress}% (✓ ${stats.successful} | ✗ ${stats.failed} | ⚡ ${stats.duplicates} duplicates)`);
          }
        }
      } finally {
        client.release();
      ...
      console.log(`   📊 Results: ${stats.successful} imported, ${stats.failed} failed, ${stats.duplicates} duplicates skipped`);
  ...
  async generateReport(outputDir) {
    ...
    const report = {
      ...
      successRate: this.importStats.totalRecords > 0 
  ...
  try {
    ...
    for (const config of IMPORT_ORDER) {
      ...
      if (config.dependencies.length > 0) {
        const unsatisfied = config.dependencies.filter(dep => 
    ...
    console.log('\n✨ Import Complete!');
    console.log('==================');
    console.log(`Total time: ${((Date.now() - importer.importStats.startTime) / 1000).toFixed(2)}s`);
