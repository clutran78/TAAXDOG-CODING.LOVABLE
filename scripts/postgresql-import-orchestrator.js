...
class ImportOrchestrator {
  ...
  async analyzeData(dataDir) {
    ...
    if (hasLargeTables) {
      analysis.recommendations.push('Some tables are very large - consider increasing database resources temporarily');
    }
  ...
  displayImportPlan(analysis) {
    ...
    console.log(`Estimated Time: ${Math.ceil(this.importPlan.reduce((sum, p) => sum + p.estimatedTime, 0) / 60)} minutes\n`);
    ...
    this.importPlan.forEach((plan, index) => {
      ...
      const deps = plan.dependencies.length > 0 ? ` (depends on: ${plan.dependencies.join(', ')})` : '';
      
      console.log(`${index + 1}. ${plan.name} - ${plan.recordCount.toLocaleString()} records, ${formatBytes(plan.size)} ${method}${deps}`);
    });
  ...
  async executeImport(dataDir, connectionString) {
    ...
    for (const plan of this.importPlan) {
      ...
      try {
        ...
        // Check dependencies
        const unmetDeps = plan.dependencies.filter(dep => 
          !this.results.collections[dep] || 
          this.results.collections[dep].successful === 0
        );
        
        if (unmetDeps.length > 0) {
          colorLog(`\n⚠️  Skipping ${plan.name}: Unmet dependencies [${unmetDeps.join(', ')}]`, 'yellow');
          continue;
        }
  ...
  calculateSummary() {
    ...
    return {
      ...
      averageSpeed: successfulRecords > 0 
        ? Math.round(successfulRecords / ((this.results.endTime - this.results.startTime) / 1000)) + ' records/s'
        : '0 records/s'
    };
  }
...
async function main() {
  ...
  try {
    ...
    if (options.interactive) {
      ...
      console.log('🔗 Database:', options.connectionString.split('@')[1]?.split('/')[0] || 'PostgreSQL');
      console.log();
