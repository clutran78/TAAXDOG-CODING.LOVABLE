...
async function runMaintenanceTasks(): Promise<MaintenanceResult> {
  ...
  try {
    ...
    const integrityCheck = await verifyAuditLogIntegrity(startOfDay(yesterday), endOfDay(today));
...
runMaintenanceTasks()
  .then((result) => {
    ...
    const allSuccess =
      result.cleanupResult.success && result.integrityResult.success && result.dailyReport.success;
