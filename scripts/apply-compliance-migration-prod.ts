...
async function applyComplianceMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(
      __dirname,
      '..',
      'prisma/migrations/20250118_add_compliance_features/migration.sql',
    );
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      const operation =
        statement.match(/^(CREATE|ALTER|DROP|INSERT|UPDATE|DELETE)\s/i)?.[1] || 'EXECUTE';
    ...
    if (finalTables.length === 6) {
      ...
      try {
        ...
        console.log(
          '   Run manually: npx prisma migrate resolve --applied 20250118_add_compliance_features',
        );
      }
    } else {
