...
async function testAuthFlow() {
  ...
  try {
    ...
    if (registerRes.ok) {
      ...
      addResult('Registration', false, `Failed: ${registerData.message || registerData.error}`, registerData);
      return;
    }
    ...
    if (!wrongLoginRes.ok && wrongLoginRes.status === 401) {
      ...
      addResult('Wrong Password', false, 'Security issue: accepted wrong password!', wrongLoginData);
    }
    ...
    if (forgotRes.ok) {
      ...
      if (resetToken) {
        ...
        if (resetRes.ok) {
          ...
          if (newLoginRes.ok) {
            addResult('Login with New Password', true, 'Login successful with new password', newLoginData);
          } else {
            addResult('Login with New Password', false, `Failed: ${newLoginData.message}`, newLoginData);
          }
        } else {
          addResult('Password Reset', false, `Failed: ${resetData.message || resetData.error}`, resetData);
        }
      } else {
    ...
    if (existingLoginRes.ok) {
      ...
      addResult('Existing User Login', false, `Failed: ${existingLoginData.message}`, existingLoginData);
    }
