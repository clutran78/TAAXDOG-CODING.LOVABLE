...
const apiKeyManager_1 = require('../lib/services/apiKeyManager');
...
async function testSecurity() {
  ...
  for (const service of services) {
    const hasKey = !!apiKeyManager_1.apiKeyManager.getApiKey(service);
    console.log(
      `${service}: ${hasKey ? '✅ Configured' : '❌ Not configured (set ${service.toUpperCase()}_API_KEY)'}`,
    );
  }
  // Test 3: Secure Headers Generation
  console.log('\n3. Testing Secure Headers:');
  try {
    const basiqHeaders = apiKeyManager_1.apiKeyManager.getSecureHeaders('basiq');
    console.log('BASIQ headers:', Object.keys(basiqHeaders));
    const stripeHeaders = apiKeyManager_1.apiKeyManager.getSecureHeaders('stripe');
