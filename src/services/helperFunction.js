...
// Firebase Firestore imports removed - using API routes instead
/*
import {
  addDoc,
  collection,
...
*/
// Firebase imports removed - using NextAuth instead
...
function updateGoalProgress(index) {
  ...
  try {
    ...
    // Show success message
    showToast(
      `Added ${formatCurrency(additionalAmount)} to "${goal.name}" successfully!`,
      'success',
    );
      ...
      goalsHTML += `
                        <div class="goal-item">
                            ...
                            <div class="d-flex justify-content-between">
                                <small>${formatCurrency(goal.currentAmount)} of ${formatCurrency(
                                  goal.targetAmount,
                                )}</small>
            ...
            <div class="card mb-4">
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <span class="text-secondary">Target: ${formatCurrency(totalTarget)}</span>
                    </div>
                </div>
            </div>
...
function createExpenseTimeChart(transactions) {
  try {
    ...
    const allDates = [
      ...new Set([...Object.keys(expensesByDate), ...Object.keys(incomeByDate)]),
    ].sort();
...
async function scanForSubscriptions(forceScan = false) {
  try {
    ...
    onAuthStateChanged(auth, async (user) => {
      ...
      const r = query(collection(db, 'subscriptions'), where('userId', '==', user?.uid));
      ...
      for (const [key, txs] of Object.entries(merchantTransactions)) {
        ...
        // const user = auth.currentUser; // Removed - using NextAuth instead
        ...
        const subscription = {
          ...
          nextPaymentDate: calculateNextPaymentDate(txs[txs.length - 1].date, avgDays)
            .toISOString()
            .split('T')[0],
...
function updateTaxProfileSummary(profile) {
  try {
    ...
    if (dependentsEl)
      dependentsEl.textContent = dependentsCount > 0 ? `${dependentsCount} dependent(s)` : 'None';
    ...
    const incomeText =
      employersCount > 0 ? `${employersCount} employer(s)` : 'No employment income';
    ...
    if (deductionsEl) {
      deductionsEl.textContent =
        deductionTypes.length > 0 ? `${deductionTypes.length} type(s)` : 'None claimed';
    }
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-select dependent-relationship" required>
                        ...
                        <option value="stepchild" ${
                          dependent?.relationship === 'stepchild' ? 'selected' : ''
                        }>Stepchild</option>
                        <option value="foster" ${
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select employer-type" required>
                        ...
                        <option value="full-time" ${
                          employer?.employmentType === 'full-time' ? 'selected' : ''
                        }>Full-time</option>
                        <option value="part-time" ${
                          employer?.employmentType === 'part-time' ? 'selected' : ''
                        }>Part-time</option>
                        <option value="casual" ${
                          employer?.employmentType === 'casual' ? 'selected' : ''
                        }>Casual</option>
                    </select>
