...
const ConnectBankForm = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      mobile: Yup.string()
        .matches(/^\+?\d{10,15}$/, 'Invalid mobile format')
  ...
  const clearOldTransactions = async (accountId: string) => {
    try {
      // Call API to clear old transactions for this account
      await apiRequest(`/api/banking/transactions/clear`, {
        method: 'POST',
        body: {
          accountId,
        },
    ...
    try {
      ...
      if (res.success) {
        ...
        const formattedTxns = rawTxns.map((txn: any) => ({
          ...
          transactionDate: txn.postDate,
          description: txn.description || 'No description',
          amount: txn.amount,
          merchant_name: txn.institution || 'Unknown',
          category: txn.class || 'Uncategorized',
          bankAccountId: accountId,
        }));

        // Clear previous transactions for that account
        await clearOldTransactions(accountId);

        // Sync transactions via API
        try {
          await apiRequest('/api/banking/transactions/sync', {
            method: 'POST',
            body: {
              accountId,
  ...
  return (
    <>
      ...
      {accounts.length > 0 && (
        <div className="shadow-md border p-6 rounded-lg mt-8">
            ...
            <ul className="space-y-4">
              {accounts.map((acc) => {
                ...
                return (
                  <li
                    key={acc.id}
                    className="border border-gray-200 p-5 rounded-lg hover:shadow-md transition-all"
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-600">
                      ...
                      <div>
                        <strong>BSB:</strong> {acc.bsb || '—'}
                      </div>
                      <div>
                        <strong>Currency:</strong> {acc.currency || 'AUD'}
                      </div>
                      <div>
                        <strong>Status:</strong> {acc.status || 'available'}
                      </div>
                      <div>
                        <strong>Institution:</strong> {acc.institution || 'AU Institution'}
                      </div>
                      <div>
                          ...
                          <div className="mt-4">
                            ...
                            <ul className="space-y-3">
                              {transactions.map((txn: any) => {
                                ...
                                const date = txn.postDate
                                  ? new Date(txn.postDate).toLocaleDateString()
                                  : '—';
                                ...
                                return (
                                  <li
                                    key={txn.id}
                                    className="bg-gray-50 border p-3 rounded text-sm"
                                  >
                                    ...
                                    <div className="text-xs text-gray-500 mt-1">
                                      {date} &middot; {txn.direction || '—'} &middot;{' '}
                                      {txn.class || 'Unclassified'}
                                    </div>
                                  </li>
                                );
                              })}
      ...
      <div className="max-w-3xl mx-auto p-6 space-y-12">
        {/* 🔹 Form */}
        <div className="shadow-xl border border-gray-100 p-8 rounded-lg transition-all duration-300">
                ...
                return (
                  ...
                  >
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-2 pr-10 rounded-md shadow-sm focus:outline-none focus:ring-2 transition duration-300 ${
                        hasError
