...
const IncomeSourceCard: React.FC<IncomeSourceCardProps> = ({ source, onClick }) => {
  const { darkMode } = useDarkMode();
  return (
    <div
      onClick={onClick}
      className="cursor-pointer card mb-3"
    >
      <div className="card-body">
        <div className="d-flex justify-content-between align-items-center">
          <h5 className={`mb-0`}>{source.name}</h5>
          <span className="badge bg-success">{source.percentage.toFixed(1)}%</span>
        </div>
        <div className="d-flex justify-content-between align-items-center mt-2">
          <div className={`${darkMode ? 'text-secondary' : 'text-muted'}`}>Monthly income</div>
