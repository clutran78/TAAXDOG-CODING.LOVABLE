 ...
 */
export const sanitizeFinancialData = (data: any): any => {
  ...
  if (Array.isArray(data)) {
    return data.map(item => sanitizeFinancialData(item));
  }
  ...
  if (typeof data === 'object') {
    ...
    for (const key in data) {
      if (data.hasOwnProperty(key)) {
        ...
        sanitized[sanitizedKey] = sanitizeFinancialData(data[key]);
      }
    }
    return sanitized;
  }
...
export const validateTransactionCategory = (category: string): boolean => {
  ...
  const validCategories = [
    ...
    'marketing', 'equipment', 'software', 'education',
    'fuel', 'repairs', 'bank-fees', 'other'
  ];
