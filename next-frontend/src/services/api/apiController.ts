...
//-----Post
export const postData = async (endpoint: string, dataObj?: any, config: any = {}) => {
    try {
...
//TO  F E T C H  D A T A  O N  S E R V E R  S I D E
 export const fetchDataOnServer = async (endpoint: string, token?: string , isCache?: boolean) => {
    try {
        const headers: HeadersInit = token ? { Authorization: `Bearer ${token}` } : {};
        const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}${endpoint}`, { headers , cache: isCache ? 'force-cache' : 'no-cache' });
        if (res.ok) {
