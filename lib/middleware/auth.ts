...
export function createUserScopedQuery<T extends Prisma.Args<any, any>>(
  userId: string,
  userRole: UserRole,
...
>(
  ...
  try {
    ...
    const hasAccess = ensureOwnership(req.userId, resource.userId, req.userRole) ||
...
export const authMiddleware = {
  ...
  authenticated: <T extends AuthenticatedRequest = AuthenticatedRequest>(
  ...
  // Require admin role
  admin: <T extends AuthenticatedRequest = AuthenticatedRequest>(
    handler: AuthHandler<T>
  ) => withAuth<T>(handler, {
    requireAuth: true,
    allowedRoles: ['ADMIN'],
  ...
  accountant: <T extends AuthenticatedRequest = AuthenticatedRequest>(
    handler: AuthHandler<T>
  ) => withAuth<T>(handler, {
    requireAuth: true,
  ...
  verified: <T extends AuthenticatedRequest = AuthenticatedRequest>(
    handler: AuthHandler<T>
  ) => withAuth<T>(handler, {
    requireAuth: true,
  ...
  optional: <T extends AuthenticatedRequest = AuthenticatedRequest>(
    handler: AuthHandler<T>
  ) => withAuth<T>(handler, {
    requireAuth: false,
    checkOwnership: true,
