...
class ApiKeyManager {
  ...
  public getApiKey(service: string): string | undefined {
    ...
    if (!apiKey) {
      logger.warn(`API key for ${service} not found in environment variables`);
      return undefined;
    }
  ...
  public rotateApiKey(service: string): string {
    ...
    logger.info(`New API key generated for ${service}. Please update your environment variables.`);
    return newKey;
  }
