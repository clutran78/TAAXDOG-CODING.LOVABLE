...
export async function updateBudgetTracking(
  ...
  try {
    ...
    if (!budget) {
      logger.info('No active budget found for user:', userId);
      return;
    }
    ...
    // Group by category
    const categoryActuals = transactions.reduce(
      (acc, t) => {
    ...
    logger.error('Update budget tracking error:', error);
    throw error;
  } finally {
...
function generateVarianceInsights(
  ...
  if (totalVariancePercent > 10) {
    insights.push(
      `You're ${Math.abs(totalVariancePercent).toFixed(0)}% over budget this month. Consider reviewing your spending.`,
    );
  } else if (totalVariancePercent < -10) {
    insights.push(
      `Great job! You're ${Math.abs(totalVariancePercent).toFixed(0)}% under budget this month.`,
    );
  } else {
  ...
  if (underBudgetCategories.length > 0) {
    ...
    insights.push(
      `You have $${totalSavings.toFixed(2)} in potential savings from under-budget categories.`,
    );
  }
...
export async function generateBudgetReport(
  ...
  try {
    ...
    const monthlyData = budget.budgetTracking.reduce(
      (acc, t) => {
