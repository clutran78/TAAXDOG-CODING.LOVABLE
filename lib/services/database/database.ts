...
class DatabaseConnection {
  ...
  private checkRateLimit(clientId: string): boolean {
    ...
    const recentAttempts = attempts.filter((timestamp) => now - timestamp < this.RATE_LIMIT_WINDOW);
  ...
  async connect(clientId: string = 'default'): Promise<void> {
    ...
    try {
      ...
      this.pool.on('error', (err) => {
        logger.error('Unexpected database pool error:', this.sanitizeError(err));
      });
  ...
  async query<T extends QueryResultRow = any>(
    ...
    try {
      ...
      if (queryLog.duration > this.SLOW_QUERY_THRESHOLD) {
        logger.warn(`Slow query detected (${queryLog.duration.toFixed(2)}ms):`, {
      ...
      if (process.env.NODE_ENV !== 'production') {
        ...
        logger.info(`Query executed (${queryLog.duration.toFixed(2)}ms):`, text);
      }
  ...
  async auditLog(operation: string, userId: string, details: Record<string, any>): Promise<void> {
    const auditEntry = {
      operation,
      user_id: userId,
      timestamp: new Date(),
    ...
    try {
      ...
      logger.error('Failed to write audit log:', this.sanitizeError(error));
    }
  }
}
