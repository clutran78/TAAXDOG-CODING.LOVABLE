...
class MigrationRunner {
  ...
  async getAppliedMigrations(): Promise<Migration[]> {
    const result = await db.query<Migration>(`SELECT * FROM ${this.tableName} ORDER BY applied_at`);
    return result.rows;
  }
  ...
  async rollback(steps: number = 1): Promise<void> {
    ...
    for (const migration of migrationsToRollback) {
      const rollbackFile = join(this.migrationsPath, 'rollback', migration.name);
