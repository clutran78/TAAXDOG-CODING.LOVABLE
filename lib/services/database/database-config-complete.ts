...
export function getDatabaseConfig(
  environment: 'production' | 'development' = process.env.NODE_ENV === 'production'
    ? 'production'
    : 'development',
...
export function getConnectionString(
  ...
  if (!connectionString && environment === 'production') {
    throw new Error(
      `${connectionType.toUpperCase()} connection string not configured. Please set the appropriate environment variable.`,
    );
  }
