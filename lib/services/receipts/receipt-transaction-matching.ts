...
function scoreMatch(receipt: any, transaction: any): MatchResult {
  ...
  const amountDiff = Math.abs(
    parseFloat(receipt.totalAmount) - Math.abs(parseFloat(transaction.amount)),
  );
  const amountScore =
    amountDiff < 0.01 ? 1.0 : Math.max(0, 1 - amountDiff / parseFloat(receipt.totalAmount));
...
export async function autoMatchReceipts(userId: string): Promise<{
  ...
  try {
    ...
    logger.error('Auto-matching error:', error);
