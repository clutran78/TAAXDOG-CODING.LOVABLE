...
function getEmailProvider(): EmailProvider {
  ...
  if (process.env.EMAIL_PROVIDER === 'sendgrid' && process.env.SENDGRID_API_KEY) {
    ...
    if (process.env.SENDGRID_API_KEY.startsWith('SG.')) {
      ...
      logger.info('[Email] ❌ Invalid SendGrid API key format, falling back to console');
      return 'console';
    }
  }
  ...
  logger.info('[Email] ⚠️ Using console provider (no email will be sent);');
  return 'console';
}
...
export async function sendEmail(options: any) {
  ...
  switch (provider) {
    ...
    case 'smtp':
      logger.info('[Email] Attempting to send via SMTP...');
      const transporter = createSMTPTransporter();
      const result = await transporter.sendMail({
        from: `${emailConfig.from.name} <${emailConfig.from.email}>`,
        ...options,
      });
      logger.info('[Email] ✅ SMTP email sent:', result.messageId);
  ...
  return {
    sendMail: async (options: any) => {
      try {
        ...
        logger.error('Error in createTransporter sendMail:', error);
        throw error;
      }
    },
  };
...
export async function sendVerificationEmail(
  ...
  logger.info('[Email] Generating verification URL:', { appUrl, verificationUrl });
...
export async function sendPasswordResetEmail(
  ...
  logger.info('[Email] Attempting to send password reset email to:', email);
  logger.info('[Email] From address:', `${emailConfig.from.name} <${emailConfig.from.email}>`);
  ...
  try {
    ...
    logger.info('[Email] ✅ Password reset email sent successfully:', result);
  } catch (error: any) {
