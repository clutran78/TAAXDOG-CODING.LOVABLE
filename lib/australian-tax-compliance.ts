...
export function calculateGST(amount: number, isInclusive: boolean = true): {
  gstAmount: number;
  netAmount: number;
  totalAmount: number;
} {
...
export function detectTaxCategory(merchant: string, items: any[]): string {
  ...
  if (merchantLower.includes('uber') || merchantLower.includes('taxi') || merchantLower.includes('cabcharge')) {
    return 'D1'; // Car expenses
  }
  
  if (merchantLower.includes('hotel') || merchantLower.includes('motel') || merchantLower.includes('airbnb')) {
  ...
  if (merchantLower.includes('telstra') || merchantLower.includes('optus') || merchantLower.includes('vodafone')) {
...
export function validateTaxInvoice(receipt: any): {
  ...
  if (!receipt.gstAmount && receipt.totalAmount > 82.50) {
