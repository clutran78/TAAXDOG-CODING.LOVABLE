...
/**
 * Type for validated API requests
 */
export interface ValidatedApiRequest<TBody = unknown, TQuery = unknown> extends NextApiRequest {
  validatedBody?: TBody;
...
export function createValidatedHandler<TBody = unknown, TQuery = unknown>(
  ...
  handler: (req: ValidatedApiRequest<TBody, TQuery>, res: NextApiResponse) => Promise<void>,
) {
  return async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    try {
      ...
      const validatedReq = req as ValidatedApiRequest<TBody, TQuery>;
      ...
      if (config.querySchema && req.query) {
        ...
        validatedReq.validatedQuery = validatedQuery;
      }
