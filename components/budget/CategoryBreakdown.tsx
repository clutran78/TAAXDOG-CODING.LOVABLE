      ...
      <div className="space-y-4">
        {categories.map((category) => {
          const progressPercent = Math.min(
            (category.actual / category.predicted) * 100,
            150
          );
          
          return (
            <div key={category.category} className="border-b border-gray-200 pb-4 last:border-0">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="text-right">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(category.status)}`}>
                    {category.status === 'over' ? 'Over' : category.status === 'under' ? 'Under' : 'On Track'}
                  </span>
