...
export const VarianceChart: React.FC<VarianceChartProps> = ({ tracking }) => {
  const chartData = useMemo(() => {
    ...
    const monthlyData = tracking.reduce((acc, t) => {
  ...
  const maxValue = Math.max(
    ...chartData.flatMap(d => [d.predicted, d.actual])
  );
    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="relative">
        ...
        <div className="flex items-end justify-between gap-2" style={{ height: '200px' }}>
          {chartData.map((data) => (
            <div key={data.key} className="flex-1 flex gap-1">
              {/* Predicted bar */}
        ...
        <div className="flex justify-between mt-2">
          {chartData.map((data) => (
            <div key={data.key} className="flex-1 text-center">
