...
const GoalCard: React.FC<GoalCardProps> = ({
  ...
  useEffect(() => {
    const loadSubaccountData = async () => {
      ...
      try {
        ...
        console.error('Failed to load subaccount summary:', error);
      } finally {
  ...
  const effectiveAmount = goal.subaccount?.useSubaccountBalance && subaccountSummary 
      ...
      <div className="card-body">
        <div className="d-flex justify-content-between align-items-start mb-3">
          <div>
            ...
            {goal.directDebit?.isEnabled && (
              <div className="mt-1">
                <small className={`${darkMode ? "text-info" : "text-primary"}`}>
                  ...
                  Next transfer: {formatNextTransferDate(goal.directDebit.nextTransferDate)} ({getTransferAmountDisplay(goal.directDebit)} {formatFrequency(goal.directDebit.frequency)})
                </small>
              </div>
            )}
            ...
            {goal.subaccount?.isEnabled && (
              <div className="mt-2 p-2 rounded" style={{ backgroundColor: darkMode ? '#1a1a1a' : '#f8f9fa' }}>
                {loadingSubaccount ? (
                  ...
                  <div className="text-center">
                    <small className="text-danger">
                      <i className="fas fa-exclamation-triangle me-1"></i>{subaccountError}
                    </small>
                  </div>
                ) : subaccountSummary ? (
                  <div>
        ...
        <div className="d-flex justify-content-between mb-2">
          <div>
            ...
            {goal.subaccount?.useSubaccountBalance && subaccountSummary && (
              <div>
                <small className="text-muted">
                  (Goal: {formatCurrency(goal.currentAmount)})
                </small>
              </div>
            )}
          </div>
        ...
        <div className="d-flex justify-content-between align-items-center">
          <div className="d-flex gap-2">
            ...
            {goal.subaccount?.isEnabled && goal.subaccount?.subaccountId && onViewSubaccountDetails && (
              <button
                className="btn btn-sm btn-outline-info"
