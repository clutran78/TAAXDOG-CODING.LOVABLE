...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Check if email is verified - but still allow password reset
    if (!user.emailVerified) {
      console.log('[ForgotPassword] Email not verified, but allowing password reset:', email);
      // Continue with password reset process even if email is not verified
    }
